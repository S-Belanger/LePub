...
- Final implementation diff reviewed: walk-in rest 10–14s, walk-in active-order patience 45–65s; regular rest 18–26s/22–30s/16–24s and active patience 54–74s/48–66s/40–54s (Nazim/Sam/Gerald). Score, spawn, shift, round, hunter, collision and art files unchanged. Research and limited workload-model evidence are in checkpoints below; no claim of human playtest.
- Final validation PASS: `node tests/smoke.js` 10 scripts/40 routes plus recovery and active-patience checks; `node tests/alex.js` 13 gates; `node tests/assets.js` 31; `node tests/character-art.js` 10 kinds/10 families/5 guards; `node --check` game.js, src/regulars.js, tests/smoke.js; `git diff --check` (Windows line-ending notices only). Real local Edge `node tools/validate-merge.js` PASS desktop 1280x720 and mobile 390x844: assets ready, status/pivots/split/pack control, zero browser errors; captures outside repo under `%TEMP%/lepub-merge-review`. No visual asset changed.
- Next: stage/review the exact five files, commit and push main, verify GitHub CI. Then deploy the linked `lepub` Vercel project with `vercel deploy --prod --yes`, verify live desktop/mobile checks and exact hashes of served `game.js` and `src/regulars.js`, record final status. Do not deploy an uncommitted gameplay tree.

## 2026-09-25 19:03 America/Toronto — solo buffer model acceptable; final checks next

- Branch `main`, HEAD `07ba2db`, upstream `origin/main` 0/0 at last check. Current local/uncommitted changes: `game.js`, `src/regulars.js`, `tests/smoke.js`, `CLAUDE.md`, `HANDOFF.md`; no push or deployment yet for this turn. User authorized production publication after completion.
- Final candidate: 10–14s walk-in post-service stay; regular next-order pauses Nazim 18–26s, Sam 22–30s, Gerald 16–24s; active-order patience walk-ins 45–65s, regulars 54–74s/48–66s/40–54s. Existing spawn, scoring, 180s first-shift, round and hunter systems unchanged. `tests/smoke.js` now also checks actual seat patience and Sam's active-order window.
- Temporary model after final candidate (four stochastic runs per service interval, hunter disabled and instant delivery): at one order/10s, 4/4 first-shift targets, ~5.1 active orders, ~1 abandonment and ~7% order-empty samples; at one order/12s, 4/4 targets, ~5.5 active orders, ~3 abandonments and ~5% order-empty samples. This supports active play with a real margin, but does not substitute for a human playtest. Node suites passed before the two newest smoke assertions: smoke 10 scripts/40 routes; Alex 13 gates; assets 31; art contract 10 kinds/10 families/5 guards.
- Next: run final suites after assertions, syntax and diff checks, inspect all source changes, fetch origin, commit/push main, deploy Vercel production and verify live desktop/mobile plus served assets. No further balance tuning planned absent a concrete failed check.

## 2026-09-25 19:01 America/Toronto — patience stress check points to a larger solo buffer

- Current local candidate is still uncommitted on `main` HEAD `07ba2db` with upstream 0/0 at last check; user's production authorization remains in force after final validation. Files changed remain `game.js`, `src/regulars.js`, `tests/smoke.js`, `CLAUDE.md`, `HANDOFF.md`; no push/deploy this turn.
- All four CI Node suites PASS after the 36–56s generic / +6s regular patience change. Four-run approximate model at 10s per delivery: shorter-rest candidate met 4/4 first-shift targets, roughly 5 active orders and 3 abandoned, with active orders 95% of sampled time. At 12s per delivery it met 0/4, roughly 5.5 active orders and 11 abandoned. This is a small stochastic instant-service model; use only as directional evidence.
- Decision: keep short 10–14s seated recovery and 16–30s regular reorders, but increase active-order patience further to create a fair solo decision window: walk-ins 45–65s; Nazim 54–74s, Sam 48–66s, Gerald 40–54s. Challenge still comes from simultaneous orders, route choices, hunter, score target and faster later shifts. Next: implement, re-run model and tests, stop tuning once the workload is active but attainable at the slower reference cadence.

## 2026-09-25 19:01 America/Toronto — patience retuned; production authorized

- User explicitly directs: after the balance/research work is done, push to production. This authorizes commit/push and deployment to the existing Vercel production project, followed by live verification. Branch `main`, HEAD `07ba2db`, upstream 0/0 at last check; all new balance work and the prior local handoff ledger are uncommitted/unpushed, no deployment yet.
- `game.js` now uses 36–56s walk-in order patience and 10–14s post-service stay. `src/regulars.js` now has active-order patience Nazim 46–66s, Sam 40–58s, Gerald 32–46s, with post-service waits 18–26s/22–30s/16–24s. `CLAUDE.md` records the current rhythm; `tests/smoke.js` checks the shorter post-service behavior. Added about six seconds of active-order patience across patrons while keeping score, spawn, shift and hunter rules untouched.
- Previous four CI suites passed after the shorter rests but before this patience adjustment; no test run on this latest version yet. Next: model a plausible service cadence again, run all Node CI suites and syntax/diff checks, inspect reviewed changes, then publish only the validated result to main and Vercel production. Live browser and served-byte verification required after deployment.

## 2026-09-25 19:00 America/Toronto — shorter rests pass; patience adjustment selected

- Branch `main`, HEAD `07ba2db`, upstream 0/0 at last check. Local only: `game.js`, `src/regulars.js`, `tests/smoke.js`, `CLAUDE.md`, existing/new `HANDOFF.md`; uncommitted/unpushed. User wants a challenge without long waits. No deployment.
- After shorter-rest edits, all four local CI suites PASS: smoke (10 scripts/40 routes including pacing), Alex (13 gates), assets (31), character art (10 kinds/10 families/5 guards). Temporary model with one completed order per 10s and no hunter/path travel: 4/4 first shifts met the 100-tip goal with the proposed rest, but averaged about 5 active orders and 7.8 abandoned orders; the previous long rest averaged 4.3 active/3.5 abandoned in the same small sample. The new rhythm has action, but the abandonment rate suggests it may punish ordinary solo routing too hard. Model is approximate and stochastic, not playtest evidence.
- Decision: retain the short recovery, give active orders a modest patience increase while preserving the relative impatience of Gerald and the timed/score challenge. PlateUp's developer explicitly increased default and solo-scaled patience during balance work (https://wiki.plateupgame.com/releasenotes/0_2). Next: implement a small patience bump, rerun focused tests and the model, then decide whether further tuning is justified. No publication yet.

## 2026-09-25 18:59 America/Toronto — challenge pacing retuned locally

- Branch `main`, HEAD `07ba2db`, upstream 0/0 at last check. Existing uncommitted `HANDOFF.md` preserved. Current local edits are `game.js`, `src/regulars.js`, `tests/smoke.js`, `CLAUDE.md` and `HANDOFF.md`; nothing committed/pushed/deployed for this turn.
- Implemented short post-service recovery: walk-ins 10–14s seated, regular reorder delay Nazim 18–26s, Sam 22–30s, Gerald 16–24s. The new values sit between the original near-instant repeat service and the previous long rest. `tests/smoke.js` timing behavior check and `CLAUDE.md` updated. Existing order patience, arrival cadence, scoring and shift goals untouched.
- No validation run after edits yet. Next: run smoke and all CI Node suites, syntax and diff checks; rerun the temporary workload model to verify no long empty periods. Then review the actual diff, record results and publish to main under the prior main-push direction if all checks pass.

## 2026-09-25 18:58 America/Toronto — pacing model reviewed; shorter rests selected

- Branch `main`, HEAD `07ba2db`, upstream `origin/main` 0/0. Existing local `HANDOFF.md` ledger preserved; no code changed or published yet this turn. User direction remains: short breathing room after service while keeping an active, challenging pub.
- Temporary out-of-repo model using the real game runtime and a simplified server completing one order every 7s: current timers reached the 100-tip first-shift target in about 51s across four runs, averaged 3.6 active orders, and had no active order for only about 10% of sampled time. A shorter 10–14s walk-in rest plus 16–30s regular rests reached the target in about 46s with a similar active queue. At a 12s/order service assumption, backlog and missed orders were high in all scenarios. These are directional stress checks only: instant delivery, no pathing skill or hunter danger, four stochastic runs each; they are not a player playtest or a balance guarantee.
- Decision: change only post-service rest to 10–14s for walk-ins and regular cooldowns to 18–26s Nazim, 22–30s Sam, 16–24s Gerald. Preserve existing active-order patience, arrival cadence, score target, round and hunter pressure. This keeps a meaningful pause for a served patron while other orders provide action; the timer change is deliberately smaller than the previous 16–24/22–44s pass.
- Next: implement timings, update focused regression and timing docs, run Node gameplay/CI checks and review source diff. No commit, push or deployment yet for this correction.

## 2026-09-25 18:56 America/Toronto — challenge-preserving pace review started

- User corrects the previous timing pass: keep LePub challenging and avoid stretches where players wait for orders; research comparable service games before retuning. Branch `main`, HEAD `07ba2db`, upstream `origin/main` 0/0 at startup; preserve the sole pre-existing uncommitted `HANDOFF.md` release ledger. No new code changes, commits, pushes or deployment yet this turn.
- Primary-source research: PlateUp describes distinct order-patience and eating phases and provides fast-eating tools and harder queue variants (https://www.plateupgame.com/updates/1-2-0-fully-loaded/ and https://www.plateupgame.com/updates/1-5-0-heat/); Overcooked 2 rewards clearing orders in sequence for score combos and offers adjustable order timers (https://www.team17.com/news/the-a-z-of-overcooked-2 and https://www.team17.com/news/overcooked-all-you-can-eat-updated-faqs). Inference for LePub: pressure should come from overlapping active orders and the shift target, with only a short post-service recovery, not a long empty wait. The exact seconds in other games do not transfer directly to LePub's room and travel distance.
- Current code has 16–24s post-delivery walk-in stay, 22–44s regular reorder intervals, 30–50s walk-in patience, six initial seat cap, 3–5.5s first-shift spawn interval, and a 180s/100-tip first shift. Next: quantify current active-order flow, choose a shorter recovery that retains pressure, implement focused behavior checks and run gameplay validation. Research alone is not a completed balance change.

## 2026-09-25 18:42 America/Toronto — drink pacing release verified

- User request fulfilled: served walk-ins now stay at their table for 16–24 seconds, even after a last-second delivery; Nazim, Sam and Gerald wait substantially longer before asking for another order, while drunk/gone Nazim still orders somewhat faster. This reduces seat turnover and repeated service trips without changing order patience, shift rules or scoring.
- Branch `main`, HEAD `07ba2db` (`fix: give patrons time between drinks`), synchronized 0/0 with `origin/main`; remote tip verified as `07ba2dbac580cdc395a1a1da48af00d40b682959`. The gameplay commit is pushed. After this checkpoint, `HANDOFF.md` is the only local uncommitted modification; no source changes after validation and no Vercel deployment requested or performed.
- Committed files: `game.js` (walk-in stay, Nazim multiplier), `src/regulars.js` (reorder intervals), `tests/smoke.js` (late-service and post-service behavior), `CLAUDE.md` (current timing), `HANDOFF.md` (pre-release checkpoints). Local PASS: `node tests/smoke.js` 10 scripts/40 routes and added pacing cases; `node tests/character-art.js` 10 kinds/10 families/5 guards; `node tests/assets.js` 31 checks; `node tests/alex.js` 13 gates; syntax checks for edited JS; staged `git diff --check`. GitHub CI run `36197888984` for `07ba2db` completed SUCCESS: https://github.com/S-Belanger/LePub/actions/runs/36197888984.
- No unfinished implementation or known functional blocker. Timing is a design choice and has automated behavior coverage, but no subjective playtest yet. Next concrete step if play feedback still finds the pub too busy: tune walk-in arrivals or initial order staggering using this pacing baseline; preserve the current tested code until then.

## 2026-09-25 18:41 America/Toronto — drink pacing pushed to main; CI running

- User's drink-service pacing change is committed as `07ba2db` (`fix: give patrons time between drinks`) and pushed to `origin/main` (`30d3b0e..07ba2db`). Remote `refs/heads/main` verified at `07ba2dbac580cdc395a1a1da48af00d40b682959`; local `main`/`origin/main` synchronized 0/0. This new `HANDOFF.md` checkpoint is the sole uncommitted local modification after the push. No Vercel deployment requested or performed.
- Changed in commit: `game.js` walk-in post-delivery 16–24s seated stay and Nazim 0.75 fast-order factor; `src/regulars.js` longer reorder intervals; `tests/smoke.js` late-delivery and regular cooldown behavior; `CLAUDE.md` timing notes; previous handoff checkpoints. All four CI Node suites passed locally, both edited JS files passed syntax checks, and staged diff check passed. Source diff reviewed before commit.
- GitHub `Validate game and character art` run `36197888984` for `07ba2db` is in progress: https://github.com/S-Belanger/LePub/actions/runs/36197888984. Next: wait for CI result and record it in a final handoff checkpoint matching the actual local Git status. The only remaining work is CI verification; no gameplay changes planned.
...
- Feature commit `0192733` (`feat: match Alex portrait and remove mace workout`) was fast-forwarded and pushed to `origin/main` (`4c47160..0192733`). Remote `refs/heads/main` verified exactly `0192733`; local `main`/`origin/main` synchronized `0/0`. GitHub `Validate game and character art` CI for `0192733` completed SUCCESS: https://github.com/S-Belanger/LePub/actions/runs/36191307154. Switched to local branch `main` at `0192733`; the only current local modification is this final `HANDOFF.md` release ledger, not yet committed/pushed.
- Release files: Alex split PNG/JSON, likeness prompt and desktop/mobile evidence; game.js/src/character-art.js/src/sound.js/manifest/test/browser-tool split-only changes; README/CLAUDE/visual-system/art notes. Deleted mace PNG/JSON/prompt and superseded mixed Alex review. Current code/docs have no mace references outside this historical handoff. Test evidence: `node tests/character-art.js` PASS 10 kinds/10 families/5 guards; `node tests/assets.js` PASS31; `node tests/smoke.js` PASS10 scripts/40 routes; `node tests/alex.js` PASS13 gates/split lifecycle. Real Edge art/Alex/merge suites PASS both 1280x720 and 390x844, 10 atlases/136 mappings, split/fallback/reduced-motion/controls, zero browser errors; native captures inspected. `git diff --check` PASS. No production Vercel deploy was requested or performed.
- Next: commit/push this docs-only ledger on `main`, verify remote tip, clean tree and CI. No remaining user-requested implementation work. Existing unused side-split anatomy limitation and lack of physical-device performance testing remain as previously documented.

## 2026-09-25 17:23 America/Toronto — split-only change committed; local main fast-forwarded

- User-authorized feature committed as `0192733` (`feat: match Alex portrait and remove mace workout`) on `feat/regulars-responsive-pixel-polish`. It contains the portrait-matched split atlas plus removal of all current mace code, assets, docs and obsolete review evidence. Commit has 57 reviewed files. Feature branch is ahead of its upstream by one; not pushed yet.
- Verified both local `main` and `origin/main` were ancestors of feature HEAD; fast-forwarded local `main` pointer to `0192733` without switching branches or rewriting history. Current branch and HEAD remain `feat/regulars-responsive-pixel-polish`/`0192733`; local main `0192733`, remote main `4c47160`. This handoff edit is the only new uncommitted local change after the feature commit.
- Final validation before commit: Node character-art/assets/smoke/Alex PASS; real Edge art/Alex/merge PASS desktop/mobile with zero errors; native captures inspected; staged whitespace check PASS. No source changes since. Next: push local `main` to `origin/main`, verify remote tip and CI, then commit/push final handoff ledger so repository history records the release. No Vercel deployment requested.
