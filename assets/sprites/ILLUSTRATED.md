...
The production manifest selects ten `*-illustrated.json` atlases: Doe,
Hunter, Nazim, Sam, Gerald, Jay (`waiter`), Alex, and three walk-in looks. These
are new raster illustrations, not exports of `ohFigure`. Built-in imagegen
produced the PNGs on September 21–25, 2026. No external image API was used.
...
stays `customer`, with its old palette retained for fallback. The ten selected
...
itself still has no runtime dependencies. Browser validation checks all ten
